...
import {
  Heart, Star, Plus, X, Bookmark, RefreshCw,
...
const CATEGORY_COLORS: Record<string, { bg: string; color: string; border: string }> = {
  financial: { bg: 'rgba(34,197,94,0.08)', color: '#22c55e', border: 'rgba(34,197,94,0.2)' },
  lifestyle: { bg: 'rgba(79,126,248,0.08)', color: '#4f7ef8', border: 'rgba(79,126,248,0.2)' },
  love: { bg: 'rgba(236,72,153,0.08)', color: '#ec4899', border: 'rgba(236,72,153,0.2)' },
  family: { bg: 'rgba(249,115,22,0.08)', color: '#f97316', border: 'rgba(249,115,22,0.2)' },
  legacy: { bg: 'rgba(168,85,247,0.08)', color: '#a855f7', border: 'rgba(168,85,247,0.2)' },
  health: { bg: 'rgba(20,184,166,0.08)', color: '#14b8a6', border: 'rgba(20,184,166,0.2)' },
  purpose: { bg: 'rgba(245,158,11,0.08)', color: '#f59e0b', border: 'rgba(245,158,11,0.2)' },
  faith: { bg: 'rgba(99,102,241,0.08)', color: '#6366f1', border: 'rgba(99,102,241,0.2)' },
  dream: { bg: 'rgba(79,126,248,0.08)', color: '#4f7ef8', border: 'rgba(79,126,248,0.2)' },
};
...
export default function VisionClient({ userId, initialGoals, initialPhotos, savedAffirmations: initialSaved }: Props) {
  ...
  // Fetch daily scripture from Bible API
  const fetchScripture = useCallback(async () => {
    setLoadingScripture(true);
    try {
      // Use a curated list of powerful book/chapter combos to get meaningful verses
      const references = [
        ...
        'genesis+1', 'john+3', 'john+14', 'romans+5', 'hebrews+11',
        'james+1', '1+corinthians+13', 'galatians+5', 'matthew+19',
        'proverbs+31', 'exodus+14',
      ];
...
Return ONLY the affirmation text, nothing else.`
  ...
  useEffect(() => {
    fetchScripture();
    generateAffirmation();
  }, [fetchScripture, generateAffirmation]);

  // Seed default goals
  async function seedGoals() {
    if (goals.length > 0 || seeded) return;
    setSeeded(true);
    ...
    const { data } = await supabase.from('vision_goals').insert(rows).select();
    if (data) setGoals(data);
  }

  useEffect(() => {
    if (initialGoals.length === 0) seedGoals();
  }, []);
  ...
  async function uploadPhoto() {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('vision').upload(path, photoFile);
      ...
      if (data) {
        ...
        setPhotoCaption('');
        toast.success('Photo added to your board! 📸');
      }
    } catch (err: any) {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        <div className="rounded-2xl p-6 relative overflow-hidden"
          style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.12) 0%, rgba(79,126,248,0.08) 100%)', border: '1px solid rgba(99,102,241,0.2)' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button onClick={fetchScripture} title="New verse"
                style={{ background: 'rgba(99,102,241,0.1)', border: 'none', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', color: '#6366f1' }}>
                <RefreshCw size={13} />
              </button>
              <button onClick={saveScripture} disabled={savingScripture} title="Save"
                style={{ background: 'rgba(99,102,241,0.1)', border: 'none', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', color: '#6366f1' }}>
                <Bookmark size={13} />
              </button>
            </div>
          ...
          {loadingScripture ? (
            ...
            <>
              <p className="text-[16px] font-medium leading-relaxed mb-3" style={{ color: 'var(--text)', fontStyle: 'italic' }}>
                "{scripture.text}"
              </p>
        ...
        {/* Affirmation */}
        <div className="rounded-2xl p-6 relative overflow-hidden"
          style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.1) 0%, rgba(249,115,22,0.06) 100%)', border: '1px solid rgba(245,158,11,0.2)' }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
            ...
            <div className="flex gap-2">
              <button onClick={generateAffirmation} title="New affirmation" disabled={loadingAffirmation}
                style={{ background: 'rgba(245,158,11,0.1)', border: 'none', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', color: '#f59e0b' }}>
                <RefreshCw size={13} className={loadingAffirmation ? 'animate-spin' : ''} />
              </button>
              <button onClick={saveAffirmation} title="Save"
                style={{ background: 'rgba(245,158,11,0.1)', border: 'none', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', color: '#f59e0b' }}>
                <Bookmark size={13} />
              </button>
            </div>
          ...
          {loadingAffirmation ? (
            ...
            <p className="text-[15px] font-medium leading-relaxed" style={{ color: 'var(--text)' }}>
              {affirmation}
            </p>
          )}
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
            {goals.map(goal => {
              ...
              return (
                <div key={goal.id} className="rounded-2xl p-5 card transition-all"
                  style={{ opacity: goal.completed ? 0.7 : 1, borderLeft: `3px solid ${colors.color}` }}>
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {goal.completed && (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                            style={{ background: 'rgba(34,197,94,0.1)', color: '#22c55e' }}>✓ Done</span>
                        )}
                      </div>
                      <h3 className="text-[15px] font-bold" style={{ color: 'var(--text)', textDecoration: goal.completed ? 'line-through' : 'none' }}>
                        {goal.title}
                      </h3>
                      {goal.description && (
                        <p className="text-[12px] mt-1" style={{ color: 'var(--text-2)' }}>{goal.description}</p>
                      )}
                    </div>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-[12px]" style={{ color: 'var(--text-2)' }}>Upload photos of your dream life — your home, your car, your destinations</p>
              </div>
            ...
            {photos.length === 0 ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
                {photos.map(photo => (
                  <div key={photo.id} className="relative rounded-2xl overflow-hidden group"
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-opacity"
                      ...
                      {photo.caption && (
                        <p className="text-white text-[12px] font-medium px-3 pb-2">{photo.caption}</p>
                      )}
                ...
                <button onClick={() => setShowPhotoModal(true)}
                  className="rounded-2xl flex items-center justify-center gap-2 transition-all"
              ...
              <div key={item.id} className="rounded-2xl p-5 card"
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {item.reference && (
                      <p className="text-[12px] font-bold mt-2" style={{ color: item.type === 'scripture' ? '#6366f1' : '#f59e0b' }}>{item.reference}</p>
                    )}
                  </div>
                  <button onClick={() => deleteSaved(item.id)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', marginLeft: 12 }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-md rounded-2xl" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-md)' }}>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Buy a Lamborghini"
                />
              </div>
              <div>
            ...
            <div className="flex justify-end gap-2 px-6 py-4" style={{ borderTop: '1px solid var(--border)' }}>
              ...
              <button onClick={addGoal} disabled={savingGoal}
                style={{ background: '#4f7ef8', border: 'none', color: '#fff', padding: '9px 20px', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 700, boxShadow: '0 2px 8px rgba(79,126,248,0.3)' }}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. My dream home in Miami"
                />
              </div>
            </div>
            <div className="flex justify-end gap-2 px-6 py-4" style={{ borderTop: '1px solid var(--border)' }}>
